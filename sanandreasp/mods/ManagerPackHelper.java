...
public class ManagerPackHelper
	{
		...
		public void checkManPack(String modname)
			{
				try
					{
						Class cls =
								Class.forName("sanandreasp.core.manpack.mod.ModContainerManPack");
						if (cls == null)
							{
								loading = false;
								stopLoad(
										String.format(
												"You forgot to install the Manager Pack!\nPlease download the latest version from:\nhttp://www.minecraftforge.net/forum/index.php/topic,2828.0.html\n- Minecraft will load without this mod: >> %s <<! -",
												modname), "No Manager Pack!");
							}
						Field verFld = cls.getDeclaredField("version");
						if (verFld != null)
							{
								int verErr =
										checkVersion((String) verFld.get(null),
												"1.9");
								if (verErr > 1)
									{
										loading = false;
		...
		public void initMan(SAPConfigManager cfgman,
				SAPLanguageManager langman, SAPUpdateManager updman)
			{
		...
		public SAPConfigManager getCfgMan()
			{
				return (SAPConfigManager) (this.cfgmanInst != null
						&& this.cfgmanInst instanceof SAPConfigManager ? this.cfgmanInst
						: null);
			}
		
		public SAPLanguageManager getLangMan()
			{
				return (SAPLanguageManager) (this.langmanInst != null
						&& this.langmanInst instanceof SAPLanguageManager ? this.langmanInst
						: null);
			}
		
		public SAPUpdateManager getUpdMan()
			{
				return (SAPUpdateManager) (this.updmanInst != null
						&& this.updmanInst instanceof SAPUpdateManager ? this.updmanInst
						: null);
			}
		...
		private int checkVersion(String currVer, String expectedVer)
			{
				...
				int majDiff =
						Integer.parseInt(currSplit[0])
								- Integer.parseInt(exptSplit[0]);
				int minDiff =
						Integer.parseInt(currSplit[1])
								- Integer.parseInt(exptSplit[1]);
				int revDiff =
						(currSplit.length < 3 ? 0 : Integer
								.parseInt(currSplit[2]))
								- (exptSplit.length < 3 ? 0 : Integer
										.parseInt(exptSplit[2]));
				...
				else if (revDiff < 0 && minDiff == 0 && majDiff == 0) { return 1; }
