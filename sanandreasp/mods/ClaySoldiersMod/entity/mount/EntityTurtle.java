		...
		private boolean handleThisWaterMovement()
			{
				return worldObj.handleMaterialAcceleration(
						boundingBox.expand(0.0D, -0.40000000596046448D, 0.0D)
				...
				if (isInWater())
					{
						...
						if (worldObj.getBlockMaterial((int) Math.floor(posX),
								(int) Math.ceil(posY - 0.5),
								(int) Math.floor(posZ)).equals(Material.water))
							{
								motionY = 0.2F;
								posY += 0.01F;
							}
		...
		public void readFromNBT(NBTTagCompound nbttagcompound)
			{
				...
				this.dataWatcher.updateObject(19,
						nbttagcompound.getShort("TurtleType"));
				...
				if (fred && this.getHealth() <= 0)
					{
						...
						if ((damagesource.equals(DamageSource.magic))
								&& !this.isKawako() && randSpec == 0)
							{
								EntityTurtle entity =
										new EntityTurtle(worldObj, posX, posY,
												posZ, this.getType());
