...
public class EntityHorse extends EntityCreature implements IMount
	{
		...
		public EntityHorse(World world, double x, double y, double z, int i)
			{
				...
				this.setAltTex(rand.nextInt(3) != 0 ? 0 : rand
						.nextInt(Textures.HORSE[i].length));
				// if (rand.nextInt(4) == 0 ) {
				// setAltSkin1(true);
				// }
				// if (rand.nextInt(5) == 0 ) {
		...
		// String epona =
		// "/sanandreasp/mods/ClaySoldiersMod/claymans/horse/horse";
		// if (this.dataWatcher.getWatchableObjectByte(18) > 0) {
		// epona = epona + "Nightmare";
		// } else if (this.dataWatcher.getWatchableObjectByte(18) > 0 &&
		// hasAltSkin2()) {
		...
		// return
		// dirtHorseTexture(this.dataWatcher.getWatchableObjectShort(19));
		// }
		...
		public void updateEntityActionState()
			{
				if (riddenByEntity == null
				...
				else
					{
						...
						moveForward =
								rider.getMoveForward()
										* (rider.getSpeedMulti());
						moveStrafing =
								rider.getMoveStrafing()
										* (rider.getSpeedMulti());
		...
		// byte byt = this.dataWatcher.getWatchableObjectByte(20);
		// this.dataWatcher.updateObject(20, (byte) (b ? (byt | 2) : (byt &
		// ~2)));
		// }
		...
		protected void dropFewItems(boolean flag, int i)
			{
				...
				dropItem(item1.itemID, 1,
						this.dataWatcher.getWatchableObjectShort(19));
			}
		...
		public EntityItem entityDropItem(ItemStack par1ItemStack, float par2)
			{
				return spawnedFromNexus || specDeath ? null : super
						.entityDropItem(par1ItemStack, par2);
			}
				...
				if (fred && this.getHealth() <= 0)
					{
						...
						if (damagesource.isFireDamage()
								&& this.dataWatcher.getWatchableObjectByte(18) <= 0
								&& this.getHealth() <= 2 && hasSpec)
							{
								EntityHorse entity =
										new EntityHorse(worldObj, posX, posY,
												posZ, this.getType());
								entity.setNightmare(true);
								entity.setAltTex(rand.nextInt(3) != 0 ? 0
										: rand.nextInt(Textures.HORSE_NIGHTMARE.length));
								// entity.texture =
								// entity.dirtHorseTexture(entity.dataWatcher.getWatchableObjectShort(19));
