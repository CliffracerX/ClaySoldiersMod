...
public class EntityClayMan extends EntityCreature implements IUpgradeEntity
	{
		...
		public EntityClayMan(World world, double xPos, double yPos,
				double zPos, int team)
			{
				this(world);
				...
				if (!SoldierTeams.getTeamByID(this.getClayTeam()).isCustom)
					this.setClayTexture(rand.nextInt(3) == 0 ? 0
							: rand.nextInt(Textures.CLAYMAN[0][this
									.getClayTeam()].length));
				else
					this.setClayTexture(SoldierTeams.getTeamByID(this
							.getClayTeam()).resLoc.length);
			}
		...
		public float getAIMoveSpeed()
			{
				return this.moveSpeed
						* (this.entityToAttack != null
								|| this.targetFollow != null ? 1.6F : 1.0F);
			}
		
		private AxisAlignedBB getTargetArea()
			{
				double radius = 8.0D;
				return AxisAlignedBB.getBoundingBox(this.posX - radius,
						this.posY - radius, this.posZ - radius, this.posX
								+ radius, this.posY + radius, this.posZ
								+ radius);
			}
		...
		public void onUpdate()
			{
				if (this.upgHash != this.upgrades.hashCode()
						...
						for (int id : this.upgrades.keySet())
							{
								par2 =
										CSMModRegistry.clayUpgRegistry
												.getUpgradeByID(id).onHit(this,
														par1DamageSource, par2);
							}
					}
				if (this.riddenByEntity != null && this.rand.nextInt(3) == 0)
					{
						this.riddenByEntity.attackEntityFrom(par1DamageSource,
								par2);
						return false;
					}
						...
						if (this.entityToAttack == null)
							{
								...
								for (EntityClayMan uberhaxornova : claymen)
									{
										if (uberhaxornova.getClayTeam() == this
												.getClayTeam())
											continue;
										if (uberhaxornova.isDead)
											continue;
										if (!this
												.canEntityBeSeen(uberhaxornova))
											continue;
										if (rand.nextInt(4) != 0)
											continue;
										...
										if (this.targetFollow != null)
											{
												if (this.targetFollow.isDead)
													this.targetFollow = null;
														...
														.canEntityBeSeen(this.targetFollow))
													this.targetFollow = null;
												else if (!hasPath()
														|| rand.nextInt(10) == 0)
														...
														for (IUpgradeItem upgrade : CSMModRegistry.clayUpgRegistry
																.getUpgrades())
															{
																...
																NBTTagCompound nbt =
																		new NBTTagCompound();
																this.upgrades
																		.put(CSMModRegistry.clayUpgRegistry
																				.getIDByUpgrade(upgrade),
																				nbt);
																upgrade.onPickup(
																		this,
		...
		public void readEntityFromNBT(NBTTagCompound par1nbtTagCompound)
			{
				...
				try
					{ // workaround for older worlds
						this.setClayTeam(par1nbtTagCompound
								.getInteger("clayTeam"));
						
						int[] textures =
								par1nbtTagCompound.getIntArray("clayTextures");
						...
						this.studded =
								par1nbtTagCompound.getInteger("studdedShield");
						this.isSuper = par1nbtTagCompound.getBoolean("super");
						this.moveSpeed = par1nbtTagCompound.getFloat("speed");
						
						this.upgrades.clear();
						NBTTagList upgList =
								par1nbtTagCompound.getTagList("upgrades");
						for (int i = 0; i < upgList.tagCount(); i++)
							{
								NBTTagCompound upgNbt =
										(NBTTagCompound) upgList.tagAt(i);
						...
						if (par1DamageSource.getEntity() instanceof EntityPlayer
							...
							
							{
								if (!par1DamageSource.isFireDamage())
									this.entityDropItem(
											new ItemStack(
													CSMModRegistry.greyDoll, 1,
													this.getClayTeam()), 0.0F);
								else
									this.entityDropItem(new ItemStack(
											CSMModRegistry.brickDoll), 0.0F);
							}
						...
						for (int id : this.upgrades.keySet())
							{
								ItemStack upgItm =
										CSMModRegistry.clayUpgRegistry
												.getUpgradeByID(id)
												.getItemStack(this);
								if (CSMModRegistry.clayUpgRegistry
										.getUpgradeByID(id).getType() != 5)
									this.entityDropItem(
											new ItemStack(upgItm.itemID, 1,
													upgItm.getItemDamage()),
											0.0F);
							}
		...
		public NBTTagCompound getUpgradeNBT(int id)
			{
				if (this.upgrades.containsKey(id)
						&& this.upgrades.get(id) == null)
