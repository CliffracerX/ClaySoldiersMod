...
public class ModelClayNexus extends ModelBase
	{
				...
				if (isSpawnable)
					{
						if (isActive)
							{
								orb.rotateAngleX =
										((EntityClayNexus) entity).innerRotation / 12F;
								orb.rotateAngleZ =
										((EntityClayNexus) entity).innerRotation / 7F;
								orb.rotateAngleY =
										((EntityClayNexus) entity).innerRotation / 10.5F;
								
								orb.rotationPointY =
										(float) (19F + Math
												.sin(((EntityClayNexus) entity).innerRotation / 15F));
							}
						
						float[] color =
								((EntityClayNexus) entity).getTeamColor();
		...
		private void
				setRotation(ModelRenderer model, float x, float y, float z)
			{
