...
public class RenderClayMan extends RenderBiped
	{
		...
		protected void
				preRenderCallback(EntityLivingBase entityliving, float f)
			{
				...
				mc1.hasArmor =
						c1.hasUpgrade(CSMModRegistry.clayUpgRegistry
				...
				// mc1.colorPadded =
				// EntityClayMan.padColorTable[c1.getPadColor()];
				//
				// if (mc1.isCaped = c1.isCaped()) {
				// mc1.capeSwing = c1.capeSwing();
				// mc1.capePadded =
				// EntityClayMan.padColorTable[c1.getCapeColor()];
				// }
				...
				// float lightness = c1.worldObj.getFullBlockLightValue(i, j,
				// k);
				// if (c1.hurtTime > 0 || c1.deathTime > 0) {
				// GL11.glColor3f((lightness / 20F) + 0.5F, 0.2F, 0.2F);
				// } else {
				// GL11.glColor3f(lightness / 35F, lightness / 35F, lightness /
				// 35F);
				// }
				// }
				
				// if (c1.holdFeather()) {
				// ItemStack itemstack = new ItemStack(Item.feather.itemID, 1,
				// 0);
				// GL11.glPushMatrix();
				//
				// float f5 = 0.625F;
				...
				//
				// renderManager.itemRenderer.renderItem(entityliving,
				// itemstack, 0);
				// GL11.glPopMatrix();
				// }
			}
		...
		protected void renderEquipped(EntityClayMan entitycm, float f)
			{
				...
				// }
				// if (entitycm.hasRightShear() && !entitycm.hasBone() &&
				// !entitycm.hasBlazeRod() && !entitycm.hasStick()) {
				...
				for (int id : entitycm.getUpgrades())
					CSMModRegistry.clayUpgRegistry.getUpgradeByID(id)
							.onRenderEquipped(entitycm, this.renderManager, f,
									this.mc1);
				...
				//
				// this.renderManager.itemRenderer.renderItem(entitycm, rightIs,
				// 0);
				// GL11.glPopMatrix();
				// }
				...
				//
				// renderManager.itemRenderer.renderItem(entitycm, itemstack,
				// 0);
				// GL11.glPopMatrix();
				// }
				// }
				// if (entitycm.hasShield()) {
				// ItemStack itemstack = new ItemStack(CSMModRegistry.shield, 1,
				// entitycm.hasEESkin() ? 2 : 0);
				// if (entitycm.isShieldStud()) { itemstack = new
				// ItemStack(CSMModRegistry.shield, 1, entitycm.hasEESkin() ? 3
				// : 1); }
				// if (itemstack != null)
				// {
				// GL11.glPushMatrix();
		...
		public void doRenderLiving(EntityLiving entityliving, double d,
				double d1, double d2, float f, float f1)
			{
				...
				for (int id : entitycm.getUpgrades())
					CSMModRegistry.clayUpgRegistry.getUpgradeByID(id)
							.onPreRender(entitycm, this.renderManager, f,
									this.mc1);
				...
				for (int id : entitycm.getUpgrades())
					CSMModRegistry.clayUpgRegistry.getUpgradeByID(id)
							.onPostRender(entitycm, this.renderManager, f,
									this.mc1);
				GL11.glPopMatrix();
			}
				...
				if (par1Entity instanceof EntityClayMan)
					{
						...
						if (clayMan.getUniqTexture() >= 0)
							{
								return Textures.CLAYMAN[2][clayMan
										.getClayTeam()][clayMan
										.getUniqTexture()];
							}
						else if (clayMan.getRareTexture() >= 0)
							{
								return Textures.CLAYMAN[1][clayMan
										.getClayTeam()][clayMan
										.getRareTexture()];
							}
						else
							{
								if (!SoldierTeams.getTeamByID(clayMan
										.getClayTeam()).isCustom)
									return Textures.CLAYMAN[0][clayMan
											.getClayTeam()][clayMan
											.getClayTexture()];
								else
									return SoldierTeams.getTeamByID(clayMan
											.getClayTeam()).resLoc[0];
							}
					}
