...
public class EntityClayCam extends EntityPlayer
	{
				...
				if (CSMModRegistry.prevPlayer != null)
					{
						...
						else if (game.renderViewEntity instanceof EntityPlayerSP
								&& !(game.renderViewEntity instanceof EntityClayCam))
							{
								CSMModRegistry.prevPlayer =
										(EntityPlayerSP) game.renderViewEntity;
								CSMModRegistry.proxy.cameraReset(game);
							}
					}
			}
		...
		public EntityClayCam(World world, EntityLiving entityliving)
			{
				...
				setPosition(entityAnchor.posX, entityAnchor.posY,
						entityAnchor.posZ);
			}
		...
		public void onUpdate()
			{
				super.onUpdate();
				if (entityAnchor == null || entityAnchor.isDead
						|| entityAnchor.getHealth() <= 0)
					{
						...
						try
							{
								...
								if (jimmy != this)
								...
								else
									{
										yOffset = 1.1F;
										double angle =
												entityAnchor.renderYawOffset
														* gnarliness;
										double d1 =
												entityAnchor.posX
														- (Math.sin(angle) * 0.8D);// 0.075D
										double d2 =
												entityAnchor.posZ
														- (Math.cos(angle) * 0.8D);
										setPosition(d1,
												entityAnchor.posY + 0.7D, d2);
