...
public class ContainerNexus extends Container
	{
		...
		public ContainerNexus(InventoryPlayer inventoryplayer,
				EntityClayNexus par2Nexus, boolean active)
			{
				nexus = par2Nexus;
				...
				if (active)
					{
						...
						for (int j = 0; j < 9 && active; j++)
							{
								addSlotToContainer(new Slot(inventoryplayer, j,
										8 + j * 18, 198));
							}
					}
			}
				...
				if (var4 != null && var4.getHasStack())
					{
						...
						if (par2 < 5 * 9 + 2 && par2 > 1)
							{
								if (!this.mergeItemStack(var5, 5 * 9 + 2,
										this.inventorySlots.size(), true)) { return null; }
							}
						else if (!this
								.mergeItemStack(var5, 2, 5 * 9 + 2, false)) { return null; }
