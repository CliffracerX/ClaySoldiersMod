...
public abstract class RightHandUpgrade implements IUpgradeItem
	{
		...
		public void onDrop(IUpgradeEntity entity, Random rnd)
			{
				entity.getEntity()
						.entityDropItem(
								new ItemStack(this.getItemStack(entity).itemID,
										this.getItemStack(entity)
												.getItemDamage(), 1), 0.0F);
			}
		...
		public boolean isCompatibleWith(IUpgradeItem upgrade)
			{
				return !(upgrade instanceof RightHandUpgrade);
			}
		...
		public float getTargetRange(IUpgradeEntity attacker)
			{
				return attacker.getEntity().ridingEntity != null ? 1.25F : 0.0F;
			}
