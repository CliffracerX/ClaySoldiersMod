...
public class UpgFlint extends MiscUpgradeND {
    ...
    public void onPickup(IUpgradeEntity entity, EntityItem item, NBTTagCompound nbt) {
    	...
    	if(cs.hasUpgrade(CSMModRegistry.clayUpgRegistry.getIDByUpgradeClass(UpgStick.class)))
    	{
        cs.playSound("step.wood",
                0.2F,
                ((cs.getRNG().nextFloat() - cs.getRNG().nextFloat()) * 0.7F + 1.0F) * 2.0F
        );
        cs.getUpgradeNBT(CSMModRegistry.clayUpgRegistry.getIDByUpgradeClass(UpgStick.class)).setBoolean("sharp", true);
    	}
    	else
    	{
    		System.out.println(cs.hasUpgrade(CSMModRegistry.clayUpgRegistry.getIDByUpgradeClass(UpgFlint.class)));
    		cs.deleteUpgrade(CSMModRegistry.clayUpgRegistry.getIDByUpgradeClass(UpgFlint.class));
    	}
    }
