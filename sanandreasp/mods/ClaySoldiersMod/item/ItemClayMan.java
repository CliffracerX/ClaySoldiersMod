...
public class ItemClayMan extends Item
	{
				...
				float par10)
			{
				if (par3World.isRemote)
				...
				else
					{
						...
						if (par7 == 1 && var11 == Block.fence.blockID
								|| var11 == Block.netherFence.blockID)
							{
								var12 = 0.5D;
							}
						...
						while (par1ItemStack.stackSize > 0 && stack > 0)
							{
								if (spawnClayMan(par3World,
										par1ItemStack.getItemDamage(),
										(double) par4 + 0.5D, (double) par5
												+ var12, (double) par6 + 0.5D)
		...
		public boolean spawnClayMan(World par0World, int par1, double par2,
				double par4, double par6)
			{
				EntityClayMan var8 =
						new EntityClayMan(par0World, par2, par4, par6, par1);
		...
		public int getColorFromItemStack(ItemStack is, int j)
			{
				if (SoldierTeams.getTeamsList().containsKey(is.getItemDamage()))
					{
						String clr =
								SoldierTeams.getTeamsList().get(
										is.getItemDamage()).itemIconStr;
		...
		public void registerIcons(IconRegister par1IconRegister)
			{
				this.itemIcon =
						par1IconRegister
								.registerIcon("claysoldiersmod:dollClay");
				for (SoldierTeams team : SoldierTeams.getTeamsList().values())
					{
						if (!team.itemIconStr.startsWith("0x"))
							team.setIcon(par1IconRegister
									.registerIcon(team.itemIconStr));
					}
			}
		...
		public Icon getIconFromDamage(int i)
			{
				return (SoldierTeams.getTeamsList().containsKey(i) && SoldierTeams
						.getTeamsList().get(i).itemIconIco != null) ? SoldierTeams
						.getTeamsList().get(i).itemIconIco : super
						.getIconFromDamage(i);
			}
