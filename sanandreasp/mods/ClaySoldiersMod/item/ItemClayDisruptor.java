...
public class ItemClayDisruptor extends Item
	{
				...
				if (CSMModRegistry.getWaveTime(entityplayer) <= 0)
					{
						...
						if (!world.isRemote
								|| (FMLCommonHandler.instance().getSide()
										.isClient() && FMLCommonHandler
										.instance()
										.getMinecraftServerInstance() == null))
							CSMModRegistry.setWaveTime(entityplayer, 100);
						if (world.isRemote)
							{
								((EntityClientPlayerMP) entityplayer).timeInPortal =
										1.5F;
								((EntityClientPlayerMP) entityplayer).prevTimeInPortal =
										1.5F;
							}
						else
							{
						...
						int y =
								MathHelper
										.floor_double(entityplayer.boundingBox.minY);
										...
										for (int k = -12; k < 13; k++)
											{
												...
												if (world.getBlockId(x + i, y
														+ j, z + k) != Block.blockClay.blockID)
													{
														continue;
													}
						...
						for (int i = 0; i < 128; i++)
							{
								...
								world.spawnParticle("portal", a, b, c, d, 0.0D,
										f);
							}
					}
				return itemstack;
		...
		private void killSoldiers(World world, EntityPlayer player)
			{
				List list1 =
						world.getEntitiesWithinAABBExcludingEntity(player,
				...
				for (int k = 0; k < list1.size(); k++)
					{
						...
						else if (entity1 instanceof EntityTurtle
								|| entity1 instanceof IMount)
							{
								entity1.attackEntityFrom(
										DamageSource.causePlayerDamage(player),
										500);
		...
		public void addBlockDestroyEffects(World worldObj, int par1, int par2,
		...
		public void registerIcons(IconRegister par1IconRegister)
			{
				this.itemIcon =
						par1IconRegister
								.registerIcon("ClaySoldiersMod:claydisruptor");
			}
	}
